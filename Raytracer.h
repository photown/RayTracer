...
{
public:
    // Casts a `Ray` for each pixel from the `World`'s camera towards each pixel on the "screen", from (0, 0) to (width, height), and gathers its color information.
    // Takes into consideration the objects in the `World`, their material properties, the `World` lights and reflections.
...
private:
    // Constructs a `Ray` in world coordinates from the `Camera` through the "screen"'s (x, y) point.
    ...
    // Populates an `Intersection` object with intersection data from shooting a given `Ray` towards `objects`, in world space.
    // To acommodate object transformations, the ray is transformed into object space, shot through the object, and the hit vector and normal are transformed back into world space.
    void Intersect(Ray& ray, KdTreeNode& kdTreeRoot, float tStart, float tEnd, std::vector<Object*>& objects, RayIntersection& result);

    // Recursively determines the color for a given `Intersection` considering other `World` data, with recursion depth determined by the depth parameter.
    ...
    void getColor(World& world, vec4& result, RayIntersection& intersection, int depth, KdTreeNode& kdTreeRoot, Box& worldBoundingBox);
};